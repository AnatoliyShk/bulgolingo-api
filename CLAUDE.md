...
- `LexemaRagService` (`services/rag_service.py`) — in-memory vector index over `lexemas`. There is nowhere to persist embeddings (no migrations in this repo), so it embeds every lexema on first use and rebuilds when the row count or newest `pk` changes; `POST /rag/index` forces a rebuild after words are edited in place. Module-level singleton `lexema_rag_service`.
